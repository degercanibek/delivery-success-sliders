...
The **Group Vote Counts** presenter control is off by default. Only explicitly enabling it shows the prominent group count cards above the chart; disabling it removes their content. Removing blur and Fully Reveal never enable this control. Reset Reveal switches it off. When enabled, cards show each group's response count, including zero. Bar labels remain average allocation scores; group counts follow the chart blur to avoid exposing identities through known group sizes. Overall response totals stay readable. Hidden identities retain their existing Group A/B aliases. Counts refresh with the chart, respect Freeze Results, and are admin-only. Changing a vote's group transfers its count and recalculates averages without increasing the total.
