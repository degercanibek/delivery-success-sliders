...
async function results(slug, current) {
  ...
  const visible = { categories: false, groups: false, values: false, axis: false, counts: false };
  const identities = identityMap(), dimensionAliases = new Map();
  let blur = 100, frozen = false, presenting = false;
  const root = layout(`<section class="results-dashboard"><header class="results-header"><div><span class="eyebrow">${tr('FARKLI BAKIŞLAR · ORTAK BAŞARI', 'DIFFERENT PERSPECTIVES · SHARED SUCCESS')}</span><h1 id="results-title">${esc(title(snapshot.session))}</h1><p class="muted">${tr('Her başarı boyutunda disiplinlerin önceliklerini karşılaştırın.', 'Compare disciplines within each success dimension.')}</p></div><div class="response-stat"><strong id="count">${snapshot.results.response_count}</strong><span>${tr('yanıt', 'responses')}</span><span id="live-status" class="live-status" role="status">${tr('Canlı', 'Live')}</span></div></header><div class="presenter-bar"><span class="muted" id="session-state"></span><div class="nav"><button class="btn" id="freeze" aria-pressed="false">${tr('Sonuçları Dondur', 'Freeze Results')}</button><button class="btn" id="presenter" aria-pressed="false">${tr('Sunucu Modu', 'Presenter Mode')}</button></div></div><div class="chart-stage"><div id="group-participation" class="group-participation" role="list" aria-label="${tr('Gruplara göre katılım', 'Participation by group')}"></div><div class="chart-scroll"><div id="chart" class="chart presentation-chart" role="img" aria-label="${tr('Başarı boyutlarına göre grup ortalamaları', 'Group averages by success dimension')}"></div></div><div id="veil-label" class="veil-label">${tr('Her bakış açısı bir hikâye anlatır.', 'Every perspective tells a story.')}<small>${tr('Hazır olduğunuzda sonuçları açın.', 'Reveal the results when you are ready.')}</small></div><p id="empty-results" class="empty-results" hidden>${tr('Henüz yanıt yok. İlk oy geldiğinde grafik otomatik güncellenecek.', 'No responses yet. The chart will update when the first vote arrives.')}</p><div class="chart-vote-count" id="chart-vote-count" role="status" aria-live="polite" aria-atomic="true"></div></div><details class="presenter-controls" open><summary>${tr('Sunum kontrolleri', 'Presentation controls')}</summary><div class="control-body"><label class="blur-control">${tr('Bulanıklık', 'Blur')} <output id="blur-value">100</output>%<input id="blur" type="range" min="0" max="100" value="100"></label><div class="reveal">${[['categories', 'Boyut Adları', 'Dimension Names'], ['groups', 'Grup Kimlikleri', 'Group Identities'], ['values', 'Kesin Değerler', 'Exact Values'], ['axis', 'Y Ekseni', 'Y Axis'], ['counts', 'Grup Oy Sayıları', 'Group Vote Counts']].map(([key, a, b]) => `<button class="btn" data-reveal="${key}" aria-pressed="false">${tr(a, b)}</button>`).join('')}</div><div class="nav preset-controls"><button class="btn" id="fully-blur">${tr('Tam Bulanıklaştır', 'Fully Blur')}</button><button class="btn primary" id="fully-reveal">${tr('Tümünü Göster', 'Fully Reveal')}</button><button class="btn" id="reset-reveal">${tr('Gösterimi Sıfırla', 'Reset Reveal')}</button></div></div></details><p id="presentation-note" class="presentation-note">${tr('Otomatik güncelleme · 3 saniye', 'Automatic updates · every 3 seconds')}</p></section>`);
  ...
  const applyBlur = () => {
    ...
    for (const element of [chartElement, root.querySelector('#group-participation')]) {
      ...
      element.setAttribute('aria-hidden', String(blur > 0 || (element !== chartElement && !visible.counts)));
    }
  ...
  const draw = (concealing = false) => {
    const participation = root.querySelector('#group-participation');
    participation.hidden = !visible.counts;
    root.querySelector('.chart-stage').classList.toggle('show-group-counts', visible.counts);
    participation.innerHTML = visible.counts ? groupParticipation(snapshot, identities, visible.groups, lang)
  ...
  root.querySelector('#fully-reveal').onclick = () => { ['categories', 'groups', 'values', 'axis'].forEach(key => visible[key] = true); blur = 0; draw(); };
